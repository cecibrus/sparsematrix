...
class matriz{
    public:
        ...
        void matricial(struct Nodo *inicio_lista, int fila, int columna);
        void agregar_elemento(struct Nodo *inicio_lista1, int fila, int columna, int elemento);
...
void matriz :: matricial(struct Nodo *inicio_lista, int fila, int columna){
        struct Nodo * lista=inicio_lista;
        cout<< "\n\nREPRESENTACION MATRICIAL" << endl;
        for(int i=0;i<fila;i++){
              for(int j=0;j<columna;j++){
                  if(lista!=NULL){
                        if(i==lista->pos_fila && j==lista->pos_columna){
                        cout<<lista->valor<<" ";
                        lista=lista->sgte;
                         }
                         else{
...
void matriz :: agregar_elemento(struct Nodo *inicio_lista1, int fila, int columna, int elemento){
    struct Nodo *lista=inicio_lista1;
    int band=0; //si es funciona con el caso 1 para que tenga que pasar por el caso general 
    //pasamos a recorrer la lista
    //CASO 1 que esas posiciones ya tienen un dato util, por lo que hay que sobreescribir
    while(lista!=NULL){
        if(fila==lista->pos_fila && columna==lista->pos_columna){
    ...
    struct Nodo * f;
    lista=inicio_lista1;
    //cout << "lista empieza  " << lista->valor << endl;
    while(lista!=NULL && band==0){
        if(fila==lista->pos_fila){
            if(fila==0 && columna < lista->pos_columna){ //cuando el elemento a insertar esta al principio de la lista                     
                f=(struct Nodo *) malloc(sizeof(struct Nodo));
                f->valor= elemento;
                f->pos_fila= fila;
                f->pos_columna= columna;
                ...
                band=1;
                lista=NULL;
            }else if(fila==3 && columna > lista->pos_columna){ //cuando el elemento a insertar esta al final de la lista
                while(lista->sgte!=NULL){ //se recorre hasta encontrar el ultimo elemento de esa fila 
                    lista=lista->sgte;
                }
                f=(struct Nodo *) malloc(sizeof(struct Nodo));
                f->valor= elemento;
                ...
                aux=lista;
                aux=aux->sgte;
                //en medio de dos elementos de la misma fila
                if((aux->pos_fila==fila && lista->pos_fila==fila) && (aux->pos_columna - lista->pos_columna) >=2){
                    f=(struct Nodo *) malloc(sizeof(struct Nodo));
                    f->valor= elemento;
                    f->pos_fila= fila;
                    f->pos_columna= columna;
                    ...
                    band=1;
                    lista=NULL;
                }else if(aux->pos_fila != lista->pos_fila){ //filas diferentes
                    f=(struct Nodo *) malloc(sizeof(struct Nodo));
                    f->valor= elemento;
                    f->pos_fila= fila;
                    f->pos_columna= columna;
                    ...
                    band=1;
                    lista=NULL;
                }else if((aux->pos_fila==fila && lista->pos_fila==fila) && (aux->pos_columna - lista->pos_columna) <=1){
                    if(aux->sgte->pos_fila != lista->pos_fila && columna < N){ //filas iguales, columnas adyacentes
                        aux=aux->sgte;
                        lista=lista->sgte;
                        f=(struct Nodo *) malloc(sizeof(struct Nodo));
                        f->valor= elemento;
...
int main(){
    ...
    mat.matricial(mat.inicio_lista,M,N);
    cout<<"\n\nNUEVA REPRESENTACION DESPUES DE AGREGAR ELEMENTO"<<endl;
    mat.agregar_elemento(mat.inicio_lista,0,1,9);  //Importante, debe probar de a uno
    //mat.agregar_elemento(mat.inicio_lista,3,4,9);
    //mat.agregar_elemento(mat.inicio_lista,0,3,9);
    //mat.agregar_elemento(mat.inicio_lista,0,2,9);
    //mat.agregar_elemento(mat.inicio_lista,3,3,9);
    mat.representacion(mat.inicio_lista, M);
